<?php
	...
	class Analyser {
		use AnalyserBaidu, AnalyserBrowserId, AnalyserCamouflage, AnalyserOperaMini, AnalyserPuffin, AnalyserUC, AnalyserUseragent, AnalyserWap;
		...
		public function __construct($options) {
			...
			if ($this->hasHeader('X-Original-User-Agent')) $this->additionalUserAgent($this->getHeader('X-Original-User-Agent'));
			if ($this->hasHeader('X-Device-User-Agent')) $this->additionalUserAgent($this->getHeader('X-Device-User-Agent'));
			if ($this->hasHeader('Device-Stock-UA')) $this->additionalUserAgent($this->getHeader('Device-Stock-UA'));
			if ($this->hasHeader('X-OperaMini-Phone-UA')) $this->additionalUserAgent($this->getHeader('X-OperaMini-Phone-UA'));
			if ($this->hasHeader('X-OperaMini-Phone')) $this->analyseOperaMiniPhone($this->getHeader('X-OperaMini-Phone'));
			if ($this->hasHeader('X-UCBrowser-Device-UA')) $this->additionalUserAgent($this->getHeader('X-UCBrowser-Device-UA'));
